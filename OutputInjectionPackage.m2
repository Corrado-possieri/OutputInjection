 ...
 for ii from 0 to ell do (
  ...
  for jj from 0 to ell-1 do (
   for vv from 0 to ell do (
    nphh = nphh + diff(kvars_(10*jj+vv),phh)*kvars_(10*jj+vv+1)*(zvars_(1)+kvars_(0));
   );
  );
  for jj from 0 to ell - 1 do (
   nphh = nphh + diff(tvars_jj,phh)*tvars_(jj+1)*(zvars_(1)+kvars_(0));
  );
  phh = nphh;
 );
...
immObs = (R,f,h,N,ell) -> (
 ...
 obId1 := substitute(ss#1,S);
 obId2 := substitute(vv#1,S);
...
immObstrNI = (R,f,h,N,ell,d) -> (
 ...
 obId1 := submatrix'(pobId1,{n},{});
 pobId2 := substitute(vv#1,S);
 obId2 := submatrix'(pobId2,{n},{});
...
immObstrNum = (R,f,h,N,ell,d,deg) -> (
 ...
 GG := transpose gens gb eliminate(ideal(soleq),kvars|{y0v}|avars);
 ...
 for ii from 0 to dimS2 - 1 do (
  Is2 = Is2 + ideal(s2Vars_(0,ii)+substitute(random(ZZ),S2));
 );
